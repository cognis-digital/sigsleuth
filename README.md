...
```mermaid
flowchart LR
  IN[input] --> P[sigsleuth<br/>analyze + score]
  P --> OUT[report]
```
...
```bash
pip install "git+https://github.com/cognis-digital/sigsleuth.git"    # pip (works today)
pipx install "git+https://github.com/cognis-digital/sigsleuth.git"   # isolated CLI
uv tool install "git+https://github.com/cognis-digital/sigsleuth.git" # uv
pip install cognis-sigsleuth                                          # PyPI (when published)
docker run --rm ghcr.io/cognis-digital/sigsleuth:latest --help        # Docker
brew install cognis-digital/tap/sigsleuth                             # Homebrew tap
curl -fsSL https://raw.githubusercontent.com/cognis-digital/sigsleuth/main/install.sh | sh
```
