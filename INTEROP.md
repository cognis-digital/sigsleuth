...
How **sigsleuth** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `sigsleuth` lives in the **General OSINT / security tool** cluster.
...
| from | relation | to |
|---|---|---|
| `sigsleuth` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis) |
| `sigsleuth` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `sigsleuth` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `sigsleuth` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the general osint / security tool cluster.** `sigsleuth` output feeds the next tool:
```bash
sigsleuth ... --format json > out.json          # this tool's findings (see `sigsleuth --help`)
...
sigsleuth ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
sigsleuth ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
sigsleuth ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `sigsleuth`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `sigsleuth` slots
