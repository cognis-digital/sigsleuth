"""Native cognis-connect emit for sigsleuth — forward findings to any platform.

Maps sigsleuth's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    sigsleuth ... --format json | sigsleuth-emit --to stix
    sigsleuth-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('intent', 'Unknown Intent')
