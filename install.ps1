# Comprehensive installer for cognis-digital/sigsleuth (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# sigsleuth is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "sigsleuth"
$Url  = "git+https://github.com/cognis-digital/sigsleuth.git"
$Git  = "https://github.com/cognis-digital/sigsleuth.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: sigsleuth"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: sigsleuth"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: sigsleuth"; exit 0 }
}
